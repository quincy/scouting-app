...
func buildApp(cfg *config.Config, db *sql.DB) (*mux.Router, context.CancelFunc) {
	...
	app.Handle("/events/{id}/replace-responsibility/{profile_id}/{responsibility}", api.RequirePermission(authService, rbacRepo, "event:signup", eventHandler.ReplaceResponsibility)).Methods("POST")
	...
	app.Handle("/events/{id}/cooking/patrols/{patrol_id}/cook/{profile_id}/replace", api.RequirePermission(authService, rbacRepo, "event:create", eventHandler.CookingReplaceCook)).Methods("POST")
	...
	app.Handle("/events/{id}/tents/override", api.RequirePermission(authService, rbacRepo, "event:create", eventHandler.TentAssignMemberOverride)).Methods("POST")
