...
### Scoutbook Sync
The process of importing roster data from Scoutbook into the app's **Profile** table. An admin pastes their Bearer JWT (obtained from `advancements.scouting.org`), and the app calls Scoutbook's `orgAdults` and `orgYouths` APIs, deduplicates by **BSA ID**, fetches email via `personprofile`, and upserts local **Profile** records. Profiles that no longer appear in Scoutbook are marked `inactive`.
